...
func NewContainerMeta(id uint64, host, port string) *globalpb.NodeMeta {
	return &globalpb.NodeMeta{id, host, port}
}
...
type Router interface {
	...
	Lookup(id uint64) *globalpb.NodeMeta
	// SetRoute set route for raft rid to node nid.
	SetRoute(rid uint64, meta *globalpb.NodeMeta)
}
...
type RaftContainer struct {
	// nodeId is the server's node id.
	*globalpb.NodeMeta
...
func NewRaftContainer(meta *globalpb.NodeMeta, router Router, grpcServer *grpc.Server) *RaftContainer {
	rc := &RaftContainer{
		NodeMeta: meta,
...
func (rc *RaftContainer) Pipe(context context.Context, m *pb.Message) (*pb.Response, error) {
	...
	if r, ok := rc.locals[m.To]; ok {
		...
		if meta := rc.router.Lookup(m.To); meta.Id != rc.Id {
			resp.Container = meta
		}
	}
	return resp, nil
...
func (rc *RaftContainer) send(m pb.Message) {
	...
	for r := util.NewRetry(retryOptions); r.Next(); {
		...
		if c != nil {
			log.Infof("RaftContainer(%d) tried to send msg to container(%d).", rc.Id, c.Id)

			if _, ok := rc.nodes[c.Id]; !ok {
				conn, err := grpc.Dial(c.Host + c.Port, grpc.WithInsecure())
				if err != nil {
					log.Errorf("Error while creating conn: %v", err)
				}
			...
			log.Infof("RaftContainer(%d) didn't found the location of raft(%d).", rc.Id, m.To)
		}
	}
}